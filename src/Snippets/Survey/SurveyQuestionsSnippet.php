    ...
    public function hasHtmlOutput(): bool
    {
        ...
        if (! $this->surveyId) {
            ...
            if ($this->trackId && (! $this->surveyId)) {
                // If a specific round was requested, show that. Otherwise, show
                // the survey of the first round.
                $roundId = $this->requestInfo->getParam('rid');
                if ($roundId) {
                    $this->surveyId = intval($this->resultFetcher->fetchOne('SELECT gro_id_survey FROM gems__rounds WHERE gro_id_track = ? AND gro_id_round = ?', [$this->trackId, $roundId]));
                } else {
