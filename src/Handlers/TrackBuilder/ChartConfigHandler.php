...
class ChartConfigHandler extends ModelSnippetLegacyHandlerAbstract
{
    ...
    {
        $model = new JoinModel('chartconfig', 'gems__chart_config', 'gcc');

        $empty = $this->translatedUtil->getEmptyDropdownArray();

        $model->set('gcc_tid', [
            'label' => $this->_('Track'),
            'multiOptions' => $empty + $this->resultFetcher->fetchPairs('SELECT gtr_id_track, gtr_track_name FROM gems__tracks ORDER BY gtr_track_name;')
        ]);
        $model->set('gcc_rid', [
            'label' => $this->_('Round'),
        ]);
        $model->set('gcc_sid', [
            'label' => $this->_('Survey'),
            'multiOptions' => $empty + $this->resultFetcher->fetchPairs('SELECT gsu_id_survey, gsu_survey_name FROM gems__surveys ORDER BY gsu_survey_name;'),
        ]);
        $model->set('gcc_code', [
            'label' => $this->_('Survey code')
        ]);
        $model->set('gcc_description', [
    ...
    "rulers":[
      ...
      },
      {
        ...
        "label":"upper"
            ...
            $model->setOnLoad('gcc_config', [$this, 'formatjson']);
        }
