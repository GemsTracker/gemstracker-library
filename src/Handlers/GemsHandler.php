...
abstract class GemsHandler extends \Zalt\SnippetsHandler\ModelSnippetHandlerAbstract
{
     ...
     * @see getSearchFilter()
     *
     * @param boolean $useSessionReadonly Use the session as only source (when false, the session is added to the input data)
     * @return array
     */
    public function getSearchData(bool $useSessionReadonly = false): array
    {
     ...
     *
     * @param boolean $useSessionReadonly Use the request as source (when false, the session is added to the input data)
    ...
    public function getSearchFilter(bool $useSessionReadonly = false): array
    {
        ...
        $filter = $this->getSearchData($useSessionReadonly);
        $this->_searchFilter = [];
        ...
        if ($action instanceof BrowseTableAction) {
            ...
            if ($action instanceof BrowseFilteredAction) {
                ...
                if ($action instanceof BrowseSearchAction) {
                    ...
                    $action->searchData = $this->getSearchData($useSessionReadonly);
                }
