...
class CommJobAction extends \Gems\Controller\ModelSnippetActionAbstract
{
    ...
    protected function createModel($detailed, $action)
    {
        ...
        \Gems\Model::setChangeFieldsByPrefix($model, 'gcj');
        ...
        if ($detailed) {
            $html = \MUtil\Html::create()->h4($this->_('Execution'));
            $model->set('execution',
                    'default', $html,
                    'label', ' ',
                    'elementClass', 'html',
        ...
        if ($detailed) {
            $html = \MUtil\Html::create()->h4($this->_('Sender'));
            $model->set('send_from',
                    'default', $html,
                    'label', ' ',
                    'elementClass', 'html',
       ...
       // RECEIVER RECEIVER RECEIVER RECEIVER RECEIVER RECEIVER RECEIVER RECEIVER RECEIVER RECEIVER RECEIVER RECEIVER
        if ($detailed) {
            $html = \MUtil\Html::create()->h4($this->_('Receiver'));
            $model->set('send_to',
                    'default', $html,
                    'label', ' ',
                    'elementClass', 'html',
        ...
        if ($detailed) {
            $html = \MUtil\Html::create()->h4($this->_('Survey selection'));
            $model->set('selection',
                    'default', $html,
                    'label', ' ',
                    'elementClass', 'html',
    ...
    {
        $jobId = intval($this->getParam(\MUtil\Model::REQUEST_ID));
    ...
    public function showAction()
    {
        ...
        if (!is_null($jobId)) {
            ...
            $cleanReq->setParam(\MUtil\Model::REQUEST_ID, null);
            $params = [
                'request'         => $cleanReq,
                'model'           => $model,
                'filter'          => $filter,
