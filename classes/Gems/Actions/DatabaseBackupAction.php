...
class DatabaseBackupAction extends \Gems\Controller\ModelSnippetActionAbstract
{
    ...
    protected function getBackupFilename($filter)
    {
        ...
        \MUtil\File::ensureDir($directory);
    ...
    public function visualBoolean($value)
    {
        if ($value === true) {
            //return '<i>O</i>';
            return \MUtil\Html::create()->i(['class' => 'fa fa-check', 'style' => 'color: green;']);
        }
        return \MUtil\Html::create()->i(['class' => 'fa fa-times', 'style' => 'color: red;']);
    }
