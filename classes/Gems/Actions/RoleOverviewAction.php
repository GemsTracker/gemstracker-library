...
class RoleOverviewAction extends \Gems\Controller\ModelSnippetActionAbstract
{
    ...
    protected function createModel($detailed, $action)
    {
        ...
        $model = new Gems\Model\PlaceholderModel('assigned-privileges', array_keys($first), $roles);
        $model->set('role',
            'label', $this->_('Role')
        );
        $model->set('parents',
            'label', $this->_('Parents'),
            'formatFunction', ['\\MUtil\\Html\\TableElement', 'createVar']
        );
        $model->set('allowed',
    ...
    protected function getRoles()
    {
        ...
        foreach ($this->acl->getRolePrivileges() as $role => $privileges) {
            ...
            $roles[$role]['parents'] = $privileges[\MUtil\Acl::PARENTS]   ? implode(', ', $privileges[\MUtil\Acl::PARENTS])   : null;
            ...
            $roles[$role]['inherited'] = $privileges[\MUtil\Acl::INHERITED][\Zend_Acl::TYPE_ALLOW] ? implode(', ', $privileges[\MUtil\Acl::INHERITED][\Zend_Acl::TYPE_ALLOW]) : null;
            //$roles[$role]['parent-denied']  = $privileges[\MUtil\Acl::INHERITED][\Zend_Acl::TYPE_DENY]  ? implode(', ', $privileges[\MUtil\Acl::INHERITED][\Zend_Acl::TYPE_DENY])  : null;
        }
        ksort($roles);
