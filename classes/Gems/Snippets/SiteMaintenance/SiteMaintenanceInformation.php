...
class SiteMaintenanceInformation extends \MUtil_Snippets_SnippetAbstract
{
    ...
    public function getHtmlOutput(\Zend_View_Abstract $view)
    {
        ...
        $div->pInfo($this->_('Site maintenance does multiple things: it link url\'s to organizations for mail and login. It also performs a security function.'));
        ...
        $p->append(' ' . $this->_('If the the url is new, then the url is by default made accessible for all organizations.'));

        $div->pInfo($this->_('After login you can use this screen to limit the organizations accessible through this url.'));

        $div->pInfo($this->_('The mail url used by an organization is the oldest url with the lowest priority number accessible for that organization.'));
        ...
        $p->append(' ' . $this->_('If the the url is new, then the url is blocked by default.'));
