 ...
 */
class Escort extends \MUtil\Application\Escort
{
    /**
    ...
    public function __construct($application)
    {
        ...
        //\MUtil\EchoOut\EchoOut::track($dirs);
        $this->_loaderDirs = array_reverse($dirs);

        foreach ($this->_loaderDirs as $prefix => $path) {
            if ($prefix) {
                \MUtil\Model::addNameSpace($prefix);
            }
        }

        // PROJECT LOADER
        $this->_projectLoader = new \MUtil\Loader\PluginLoader($this->_loaderDirs);
    ...
    public function _($text, $locale = null)
    {
        if (! isset($this->request)) {
            ...
            $this->setException(new \Gems\Exception\Coding('Requested translation before request was made available.'));
        }
...
//                $this->logger->log($msg, \Zend_Log::ERR);
    ...
    protected function _copyVariables($object)
    {
        ...
        foreach ($this->getContainer() as $key => $value) {
            ...
            if ($value !== $object) {
                // \MUtil\EchoOut\EchoOut::r(get_class($value), $key);
     ...
     *
     * @return \Gems\Project\ProjectSettings
     */
    ...
    protected function _initCache()
    {
        ...
        if (\MUtil\Console::isConsole() && !ini_get('apc.enable_cli') && $useCache === 'apc') {
    ...
    protected function _initLoader()
    {
        ...
        \MUtil\Model::setSource($loader, true);
    ...
    {
        \Zend_Controller_Action_HelperBroker::addPrefix('Gems\Controller\Action_Helper');
    }
     ...
     *
     * @return \Gems\Util\BasePath
     */
    protected function _initBasepath()
    {
        return $this->createProjectClass('Util\\BasePath');
    }
    ...
    {
        $transport = new \Gems\Mail\Transport\SendMail();
    ...
    protected function _layoutContact(array $args = null)
    {
        if ($this->menu instanceof \Gems\Menu) {
            ...
            if ($menuItem) {
                $contactDiv = \MUtil\Html::create()->div(
                    $args,
                    array('id' => 'contact')
                );  // tooltip
        ...
        if ($this->menu && $this->menu->isVisible()) {
            ...
            if ($path || (isset($args['always']) && $args['always'])) {
                ...
                if ($this->useBootstrap && !isset($args['tag'])) {
                    $div = \MUtil\Html::create('ol', $args + array('id' => 'crumbs', 'class' => 'breadcrumb'));
                    ...
                    $div = \MUtil\Html::create($tag, $args + array('id' => 'crumbs'));

                    $content = $div->seq();
                    $content->setGlue(\MUtil\Html::raw($this->_(' > ')));
    ...
    protected function _layoutEnvironment(array $args = null)
    {
        ...
        return \MUtil\Html::create($tagName, $env, $args);
    }
    ...
    {
        // JQUERY
        if (\MUtil\JQuery::usesJQuery($this->view)) {
    ...
    protected function _layoutLocaleSet(array $args = null)
    {
        ...
        $localeDiv = \MUtil\Html::create('div', $args, array('id' => 'languages'));
    ...
    protected function _layoutLogin(array $args = null)
    {
        ...
        if ($this->currentUser && $this->menu) {
            $div = \MUtil\Html::create('div', array('id' => 'login'), $args);
    ...
    protected function _layoutNavigation()
    {
        ...
        if ($this->menu && $this->menu->isVisible()) {
            ...
            // $zendNav->setAcl($this->acl);  // Not needed with \Gems\Menu
    ...
    protected function _layoutOrganizationName(array $args = null)
    {
        ...
        unset($args['addEnv']);
        return \MUtil\Html::create($tagName, $name, $args);
    }
    ...
    {
        return \MUtil\Html::create()->div(date('d-m-Y H:i:s'), $args, array('id' => 'time'));
    }
    ...
    protected function _layoutTitle(array $args = null)
    {
        ...
        if ($this->controller instanceof \MUtil\Controller\Action) {
    ...
    {
        $div = \MUtil\Html::create()->div($args, array('id' => 'version'));
        if ($this->currentUser->isActive()) {
        ...
        if (($this->menu instanceof \Gems\Menu) &&
            ...
            $link = \MUtil\Html::create()->a($version, $item);
        } else {
            $link = $version;
        }
    ...
    public function dispatchLoopStartup(\Zend_Controller_Request_Abstract $request)
    {
        ...
        if (! isset($this->db)) {
            $this->setException(new \Gems\Exception\Coding(
    ...
    /**
     * Retrieve the \Gems\Escort object
     *
     * @return \Gems\Escort
     */
     ...
     *
     * @return \Gems\Loader Or a subclassed version when specified in the project code
     */
    ...
    {
        $uiSwitch  = \MUtil\Html::create('div', $args, array('id' => $elementId));
        $params    = $this->request->getparams();
        unset($params['error_handler']);    // If present, this is an object and causes a warning
        unset($params[\MUtil\Model::AUTOSEARCH_RESET]);
     ...
     *
     * @return \Gems\Util Or a subclassed version when specified in the project code
     */
...
//        $host = \MUtil\StringUtil\StringUtil::stripToHost($fullHost);
//        $request = $this->request;
//        if ($request instanceof \Zend_Controller_Request_Http) {
//            if ($host == \MUtil\StringUtil\StringUtil::stripToHost($request->getServer('HTTP_HOST'))) {
//                return true;
//            }
//        }
//        if (isset($this->project)) {
//            foreach ($this->project->getAllowedHosts() as $allowedHost) {
//                if ($host == \MUtil\StringUtil\StringUtil::stripToHost($allowedHost)) {
//                    return true;
//                }
//            }
//        }
...
//            if ($host == \MUtil\StringUtil\StringUtil::stripToHost($url)) {
//                return true;
//            }
//        }
//
//        return false;
            ...
            foreach ($headers as $name => $value) {
                ...
                if ($name == 'Content-Security-Policy' && strpos($value, '$scriptNonce') !== false) {
                    if (Javascript::$scriptNonce) {
                        ...
                        throw new Gems\Exception\Security('script nonce set in headers but no nonce generated');
                    }
                } // */
    ...
    public function prepareController()
    {
        ...
        if ($this->useBootstrap) {
            $bootstrap = \MUtil\Bootstrap::bootstrap(array('fontawesome' => true));
            \MUtil\Bootstrap::enableView($this->view);
        }
        ...
        foreach ($layoutFuncs as $prepare => $type) {
            ...
            if (is_numeric($type)) {
                ...
                if (!isset($this->view->$type)) {
                    $this->view->$type = new \MUtil\Html\Sequence();
                }
    ...
    public function requestChanged(\Zend_Controller_Request_Abstract $request)
    {
        if ($this->project->isMultiLocale()) {
            // Get the choosen language
            $localeId = \Gems\Cookies::getLocale($request);
            if (! $localeId) {
            ...
            if ($localeId && ($this->locale->getLanguage() !== $localeId)) {
                // \MUtil\EchoOut\EchoOut::r('On cookie ' . $localeId . ' <> ' . $this->locale->getLanguage());
        ...
        if (\MUtil\Bootstrap::enabled() && $this->project->isBootstrapLocal()) {
            $bootstrap = \MUtil\Bootstrap::bootstrap();
    ...
    public function routeShutdown(\Zend_Controller_Request_Abstract $request)
    {
        ...
        if ($this->getUtil()->getMaintenanceLock()->isLocked()) {
            if ($user->hasPrivilege('pr.maintenance.maintenance-mode', false)) {
                \MUtil\EchoOut\EchoOut::r($this->_('System is in maintenance mode'));
        ...
        if (('index' == $controller) &&
            ...
            if (! ($menuItem && $menuItem->get('allowed'))) {
                if ($menuItem) {
                    ...
                    if ($alternative && \MUtil\StringUtil\StringUtil::contains($alternative, '/')) {
                ...
                if ($user->getUserId()) {
                    ...
                    if ($action == 'autofilter') {
                        ...
                        throw new \Gems\Exception("Session expired", 401);
                    }
                    ...
                    if ($menuItem) {
                        ...
                        if (! ($controller == 'index' && $action == 'logoff')) {
                            ...
                            if (! \MUtil\StringUtil\StringUtil::contains($controller . $action, '.')) {
    ...
    public function setControllerDirectory(\Zend_Controller_Request_Abstract $request)
    {
        ...
        // \MUtil\EchoOut\EchoOut::r(APPLICATION_PATH . '/controllers/' . $controllerFileName);
    ...
    {
        if ($isSecurity) {
            $e = new \Gems\Exception\Security($message, $code, null, $info);
        } else {
            $e = new \Gems\Exception($message, $code, null, $info);
        }
        $this->setException($e);
    }
