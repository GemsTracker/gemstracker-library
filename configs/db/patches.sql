...
UPDATE gems__roles
    SET grl_privileges =
        CONCAT(grl_privileges,',pr.status-code,pr.status-code.create,pr.status-code.delete,pr.status-code.edit')
    WHERE grl_name = 'super' AND grl_privileges NOT LIKE '%pr.status-code%';
